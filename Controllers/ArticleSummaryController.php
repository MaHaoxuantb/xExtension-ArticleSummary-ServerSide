...
final class FreshExtension_ArticleSummary_Controller extends Minz_ActionController {
  ...
  private function runAiRequestWithPrompt(mixed $oai_prompt): void {
    ...
    if ($oai_provider === 'openai') {
      $requestUrl = $oai_url . '/chat/completions';
      ...
      // Ollama
      $requestUrl = $oai_url . '/api/generate';
      if (!empty($oai_key)) {
        $headers[] = 'Authorization: Bearer ' . $oai_key;
      }
      $payload = [
  ...
  /**
   * Send a streaming request to the AI provider and pass the output to the client
   * 向AI提供商发送流式请求并将输出传递给客户端
   * 
   * @param string $url The API URL
   * @param array $headers The request headers
   * @param array $payload The request payload
   */
  private function streamRequest(string $url, array $headers, array $payload): void {
    // Open a connection to the AI provider
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($payload));
    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, false); // Do not return the response, stream it directly
    
    $isError = false;
    $errorBody = '';
    $headersSent = false;

    curl_setopt($ch, CURLOPT_WRITEFUNCTION, function($ch, $data) use (&$isError, &$errorBody, &$headersSent) {
      ...
      if (!$headersSent) {
          // Headers are already set in summarizeAction, but we ensure they are sent now
          $headersSent = true;
      }
    ...
    if ($isError) {
        // If it was an HTTP error, we buffered the response instead of streaming it
        // We can now send a JSON error response
        $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
      ...
      // Handle curl error - create a fake error response that the client can parse
      // 处理错误 - 创建一个客户端可以解析的伪错误响应
      $errorMsg = curl_error($ch);
      // Depending on the format (OpenAI or Ollama), this might need adjustment
